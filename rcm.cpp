...
double average(vector<double> d)
{
    ...
    double avg = sum / (int)d.size();
    return avg;
}


double stdev(vector<double> d)
{
    double sum = 0.0;
    for (int i = 0; i < d.size(); i++) sum += d[i];
    double avg = sum / (int)d.size();
    sum = 0.0;
    for (int i = 0; i < d.size(); i++) sum += fabs(d[i] - avg);
    sum /= (int)d.size();
    return sqrt(sum);
}
...
int main()
{
    ...
    for (sns = 0; sns < 13; sns += 2)
    {
        ...
        while (num--)
        {
            ...
            int old = old_paper();
            old_arr.push_back(100.0*(double)old/old);
